...
Quad::Quad(const Level* l, int64_t i, int64_t j)
    : wi(l->wi),dim(l->dim),lopt(l->lopt),level(l),g(&l->g),sn(&l->sn),highres(l->highres),scale(l->scale)
...
void Quad::run()
{
    ...
    if (pixs > 0)
    {
        ...
        if (wi.mc == MC_BEDROCK && dim == DIM_OVERWORLD && level && level->world && level->world->bedrock_state)
        {
            biomes = (int *)malloc((size_t)w * h * sizeof(int));
            if (!biomes) return;
            int err = bedrock_gen_biomes(level->world->bedrock_state, biomes, x, z, w, h);
            if (err)
            {
                for (int i = 0; i < w*h; i++)
                    biomes[i] = -1;
...
void Level::init4map(QWorld *w, int pix, int layerscale)
{
    ...
    if (wi.mc == MC_BEDROCK)
        setupGenerator(&g, MC_1_17, 0);  /* Level g kept valid; overworld biomes from world->bedrock_state */
    else if (lopt.mode == LOPT_RIVER_4 && wi.mc >= MC_1_13 && wi.mc <= MC_1_17)
    {
        setupGenerator(&g, wi.mc, wi.large);
    ...
    , qual()
{
    if (wi.mc == MC_BEDROCK)
    {
        bedrock_state = bedrock_create(wi.bedrock_mc);
        if (bedrock_state)
            bedrock_apply_seed((bedrock_state_t)bedrock_state, wi.seed);
        setupGenerator(&g, MC_1_17, 0);  /* keep g valid for Nether/End and getBiome */
    }
    else
        setupGenerator(&g, wi.mc,  wi.large);
...
QWorld::~QWorld()
{
    ...
    if (bedrock_state)
    {
        bedrock_destroy((bedrock_state_t)bedrock_state);
        bedrock_state = nullptr;
    }
...
void QWorld::setDim(int dim, LayerOpt lopt)
{
    ...
    this->dim = dim;
    this->lopt = lopt;
    if (wi.mc == MC_BEDROCK && bedrock_state && dim == DIM_OVERWORLD)
        bedrock_apply_seed((bedrock_state_t)bedrock_state, wi.seed);
    applySeed(&g, dim, wi.seed);
...
{
    if (wi.mc == MC_BEDROCK && dim == DIM_OVERWORLD && bedrock_state)
    {
        int buf[1];
        if (bedrock_gen_biomes((bedrock_state_t)bedrock_state, buf, p.x / 4, p.z / 4, 1, 1) == 0)
            return buf[0];
        return -1;
    }
