...
void MapView::setBedrockMode(bool bedrock)
{
    if (bedrockMode != bedrock)
    {
        bedrockMode = bedrock;
        update(2);
    }
...
void MapView::paintEvent(QPaintEvent *)
{
    ...
    if (world)
    {
        ...
        // Show "Java generation" banner only when Bedrock is selected but we're not drawing Bedrock
        if (bedrockMode && world->wi.mc != MC_BEDROCK)
        {
            QString msg = tr("Bedrock Edition selected — map shows Java generation. For Bedrock world gen use ChunkBase or similar tools.");
            QRect r = painter.fontMetrics().boundingRect(0, 0, width(), height(), Qt::TextWordWrap | Qt::AlignCenter, msg);
            r.adjust(-12, -6, 12, 6);
            r.moveTop(8);
            r.setLeft(8);
            r.setRight(width() - 8);
            painter.fillRect(r, QBrush(QColor(40, 44, 52, 220), Qt::SolidPattern));
            painter.setPen(QColor(255, 213, 79));
            painter.drawText(r, Qt::TextWordWrap | Qt::AlignCenter, msg);
        }
