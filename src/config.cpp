...
{
    return mc == wi.mc && large == wi.large && seed == wi.seed && y == wi.y && edition == wi.edition && bedrock_mc == wi.bedrock_mc;
}
...
void WorldInfo::reset()
{
    ...
    seed = 0;
    y = 255;
    edition = EDITION_JAVA;
    bedrock_mc = 17;  // 1.17, matches Bedrock lib MC_NEWEST
}
...
void WorldInfo::load(QSettings& settings)
{
    ...
    edition = settings.value("map/edition", edition).toInt();
    bedrock_mc = settings.value("map/bedrock_mc", bedrock_mc).toInt();
}
...
void WorldInfo::save(QSettings& settings)
{
    ...
    settings.setValue("map/edition", edition);
    settings.setValue("map/bedrock_mc", bedrock_mc);
}
...
void Config::load(QSettings& settings)
{
    ...
    edition = settings.value("config/edition", edition).toInt();
...
void Config::save(QSettings& settings)
{
    ...
    settings.setValue("config/edition", edition);
