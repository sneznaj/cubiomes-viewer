...
/* Bedrock version combo: 1.6–1.17 (library-supported). */
static const int BEDROCK_MC_MIN = 6, BEDROCK_MC_MAX = 17;
static QString bedrockMc2Str(int bedrock_mc)
{
    if (bedrock_mc < BEDROCK_MC_MIN || bedrock_mc > BEDROCK_MC_MAX)
        bedrock_mc = BEDROCK_MC_MAX;
    return QString::number(1) + "." + QString::number(bedrock_mc);
}
static int bedrockStr2Mc(const QString &s)
{
    bool ok;
    int v = s.section('.', 1, 1).toInt(&ok);
    if (ok && v >= BEDROCK_MC_MIN && v <= BEDROCK_MC_MAX)
        return v;
    return BEDROCK_MC_MAX;
}
...
{
    bool ok = true;
    wi->edition = config.edition;

    if (config.edition == EDITION_BEDROCK)
    {
        wi->mc = MC_BEDROCK;
        wi->bedrock_mc = bedrockStr2Mc(ui->comboBoxMC->currentText());
    }
    else
    {
...
bool MainWindow::setSeed(WorldInfo wi, int dim)
{
    ...
    ui->comboBoxEdition->setEnabled(false);
    ui->checkLarge->setEnabled(false);
    ui->seedEdit->setEnabled(false);
    ui->comboY->setEnabled(false);

    config.edition = wi.edition;
    ui->comboBoxEdition->setCurrentIndex(wi.edition);
    ...
    if (wi.mc == MC_BEDROCK)
        ui->comboBoxMC->setCurrentText(bedrockMc2Str(wi.bedrock_mc));
    else
        ui->comboBoxMC->setCurrentText(mc2str(wi.mc));
    ui->seedEdit->setText(QString::asprintf("%" PRId64, (int64_t)wi.seed));
    getMapView()->setSeed(wi, dim, lopt);
    getMapView()->setBedrockMode(config.edition == EDITION_BEDROCK);

    ui->comboBoxMC->setEnabled(true);
    ui->comboBoxEdition->setEnabled(true);
...
void MainWindow::loadSettings()
{
    ...
    onUpdateMapConfig();
    ui->comboBoxEdition->setCurrentIndex(config.edition);
    setSeed(wi, dim);
...
void MainWindow::setMCList(bool experimental)
{
    ...
    if (config.edition == EDITION_BEDROCK)
    {
        for (int v = BEDROCK_MC_MIN; v <= BEDROCK_MC_MAX; v++)
            mclist.append(bedrockMc2Str(v));
        if (wi.mc != MC_BEDROCK)
            wi.bedrock_mc = BEDROCK_MC_MAX;
    }
    else
    {
    ...
    QString s = (wi.mc == MC_BEDROCK) ? bedrockMc2Str(wi.bedrock_mc) : mc2str(wi.mc);
    ui->comboBoxMC->setEnabled(false);
    ui->comboBoxMC->clear();
...
void MainWindow::on_comboBoxEdition_currentIndexChanged(int index)
{
    if (ui->comboBoxEdition->isEnabled())
    {
        config.edition = index;
        config.save();
        setMCList(g_extgen.experimentalVers);
        getMapView()->setBedrockMode(config.edition == EDITION_BEDROCK);
        updateMapSeed();
