...
# compile cubiomes (Java) then cubiomes-bedrock as shared lib (loaded at runtime to avoid symbol clash)
CUPATH              = $$PWD/cubiomes
CUBEDPATH           = $$PWD/cubiomes-bedrock
QMAKE_PRE_LINK      += $(MAKE) -C $$CUPATH -f $$CUPATH/makefile CC=\"$$QMAKE_CC\" CFLAGS=\"$(CFLAGS) $$QMAKE_CFLAGS\" $$CUTARGET && \
    $(MAKE) -C $$CUBEDPATH -f $$CUBEDPATH/Makefile CC=\"$$QMAKE_CC\" CFLAGS=\"$(CFLAGS) $$QMAKE_CFLAGS\" $$CUTARGET
QMAKE_CLEAN         += $$CUPATH/*.o $$CUPATH/libcubiomes.a $$CUBEDPATH/*.o $$CUBEDPATH/libcubiomes-bedrock.so $$CUBEDPATH/libcubiomes-bedrock.dylib
LIBS                += $$CUPATH/libcubiomes.a -lm
# Copy Bedrock shared library next to executable so wrapper can dlopen it
macx: QMAKE_POST_LINK += cp -f $$CUBEDPATH/libcubiomes-bedrock.dylib $$OUT_PWD/cubiomes-viewer.app/Contents/MacOS/ 2>/dev/null || true
else: unix:!macx: QMAKE_POST_LINK += cp -f $$CUBEDPATH/libcubiomes-bedrock.so $$OUT_PWD/ 2>/dev/null || true
